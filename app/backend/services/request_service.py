...
async def create_join_request(user: Dict[str, Any], pool_id: str) -> Dict[str, Any]:
    ...
    try:
        await db.users.update_one(
            ...
            {"$set": {"temp_requester_name": request_doc["requester_name"]}},
        )
        await send_push(
            pool["user_id"],
            "New ride request",
...
async def accept_request(request_id: str, user_id: str) -> Optional[Dict[str, Any]]:
    ...
    if pool:
        ...
        # The group chat is shared by the route/time window. If one already
        # exists because the matching algorithm found people, this adds the
        # accepted traveler to that same chat; otherwise it creates one now.
        try:
            from services.messages_service import ensure_trip_conversation
            conversation = await ensure_trip_conversation(
                req_doc["pool_id"], [req_doc["requester_id"]]
            )
        except Exception as e:
