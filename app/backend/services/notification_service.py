...
async def send_match_notifications(pool: Dict[str, Any], matches: List[Dict[str, Any]]) -> None:
    ...
    for match in matches:
        ...
        if pool.get("user_email"):
            jobs.append(send_email_notification(
                pool["user_email"],
                "UniPool: New match found",
                f"<html><body><p>{owner_body}</p><p>Open UniPool to coordinate the trip.</p></body></html>",
            ))
        if match.get("user_email"):
            jobs.append(send_email_notification(
                match["user_email"],
                "UniPool: New match found",
                f"<html><body><p>{match_body}</p><p>Open UniPool to coordinate the trip.</p></body></html>",
            ))
