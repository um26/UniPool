...
async def route_heatmap(authorization: Optional[str] = Header(None)):
    """Return anonymized route frequency and local-hour demand buckets."""
    await _user(authorization)
    ...
    routes = [
        {"from": route_labels[key][0], "to": route_labels[key][1], "count": count}
        for key, count in route_counts.most_common(40)
    ]
    return {
        "routes": routes,
        "hourly": [{"hour": hour, "count": count} for hour, count in enumerate(hourly)],
...
async def submit_report(body: ReportCreate, authorization: Optional[str] = Header(None)):
    ...
    reported = await db.users.find_one(
        {"user_id": body.reported_user_id}, {"_id": 0, "user_id": 1, "name": 1}
    )
    if not reported:
    ...
    doc = {
        ...
        "reporter_name": user.get("name") or "Traveller",
        "reported_user_id": body.reported_user_id,
        "reported_user_name": reported.get("name") or "Traveller",
        "reason": body.reason,
...
async def remove_confirmed_traveler(
    pool_id: str,
    traveler_user_id: str,
    ...
    is_self_leave = traveler_user_id == uid and any(
    ...
    result = await db.pools.update_one(
        ...
        {"$pull": {"confirmed_travelers": {"user_id": traveler_user_id}}},
    )
    ...
    if conversation_id:
        await db.conversations.update_one(
            ...
            {
                "$pull": {"member_ids": traveler_user_id},
