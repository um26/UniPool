...
export const api = {
  ...
  submitRating: (rated_user_id: string, stars: number, comment?: string, pool_id?: string) =>
    req("/profile/ratings", { method: "POST", body: JSON.stringify({ rated_user_id, stars, comment, pool_id }) }),
  getUserRatings: (userId: string) => req(`/profile/ratings/${userId}`),
  canRate: (userId: string) => req(`/profile/ratings/can-rate/${userId}`),
  requestToJoin: (poolId: string) => req(`/requests/pools/${poolId}/requests`, { method: "POST" }),
  listPoolRequests: (poolId: string) => req(`/requests/pools/${poolId}/requests`),
  ...
  blockUser: (userId: string) => req(`/profile/users/${userId}/block`, { method: "POST" }),
  unblockUser: (userId: string) => req(`/profile/users/${userId}/block`, { method: "DELETE" }),
  listBlocked: () => req("/profile/users/me/blocked"),
  ...
  verifyCollegeIdStart: (collegeEmail: string) => req("/auth/profile/verify-college-id/start", { method: "POST", body: JSON.stringify({ college_email: collegeEmail }) }),
  verifyCollegeIdConfirm: (code: string) => req("/auth/profile/verify-college-id/confirm", { method: "POST", body: JSON.stringify({ code }) }),
};
