...
export default function BrandFooter({ light = false }: { light?: boolean }) {
  ...
  return <View style={styles.wrap}>
    ...
    <View style={styles.row}><Text style={[styles.text, { color }]}>Made with </Text><Ionicons name="heart" size={12} color={light ? colors.saffron : colors.error} /><Text style={[styles.text, { color }]}> by BinaryBots</Text><Pressable testID="brand-instagram-link" onPress={() => Linking.openURL(INSTAGRAM_URL)} style={styles.igRow} hitSlop={8}><Text style={[styles.text, { color }]}>  ·  </Text><Ionicons name="logo-instagram" size={14} color={color} /><Text style={[styles.text, { color, marginLeft: 3 }]}>@{INSTAGRAM_HANDLE}</Text></Pressable></View>
  </View>;
}
...
const styles = StyleSheet.create({
  ...
  row: { flexDirection: "row", alignItems: "center", justifyContent: "center", flexWrap: "wrap", paddingVertical: SPACING.md }, igRow: { flexDirection: "row", alignItems: "center" }, text: { fontSize: FONT.sm },
});
