...
export default function BrandFooter({ light = false }: { light?: boolean }) {
  ...
  if (onProfile) return <View style={styles.wrap}><View style={styles.profileLinks}>
    <Pressable accessibilityLabel="Open your Travel Network" onPress={() => router.push("/network" as any)} style={[styles.network, { backgroundColor: colors.card, borderColor: colors.border }]}><View style={[styles.networkIcon, { backgroundColor: colors.surface2 }]}><Ionicons name="git-network-outline" size={18} color={colors.indigo} /></View><View style={{ flex: 1 }}><Text style={[styles.networkTitle, { color: colors.onSurface }]}>Travel Network</Text><Text style={[styles.networkSub, { color: colors.muted }]}>Reliability, completed rides and history</Text></View><Ionicons name="chevron-forward" size={17} color={colors.muted} /></Pressable>
    <Pressable accessibilityLabel="Open UniPool settings" onPress={() => router.push("/settings" as any)} style={[styles.network, { backgroundColor: colors.card, borderColor: colors.border }]}><View style={[styles.networkIcon, { backgroundColor: colors.surface2 }]}><Ionicons name="settings-outline" size={18} color={colors.saffron} /></View><View style={{ flex: 1 }}><Text style={[styles.networkTitle, { color: colors.onSurface }]}>Settings</Text><Text style={[styles.networkSub, { color: colors.muted }]}>Notifications, appearance, privacy and help</Text></View><Ionicons name="chevron-forward" size={17} color={colors.muted} /></Pressable>
  </View></View>;
...
const styles = StyleSheet.create({
  wrap: { paddingHorizontal: SPACING.lg, paddingTop: SPACING.md }, profileLinks: { width: "100%", maxWidth: 920, alignSelf: "center", flexDirection: "row", flexWrap: "wrap", gap: 9 },
  network: { flexGrow: 1, flexBasis: 280, minHeight: 72, borderWidth: 1, borderRadius: 18, padding: 11, flexDirection: "row", alignItems: "center", gap: 10 }, networkIcon: { width: 40, height: 40, borderRadius: 13, alignItems: "center", justifyContent: "center" }, networkTitle: { fontSize: 12, fontWeight: "900" }, networkSub: { fontSize: 9, marginTop: 2 },
