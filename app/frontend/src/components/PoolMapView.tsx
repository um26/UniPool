...
export default function PoolMapView({ pools }: { pools: Pool[] }) {
    ...
    (async () => {
      try {
        ...
        for (const pool of uniquePools) {
          ...
          if ((!canonicalFrom || !canonicalTo) && !cancelled) await new Promise((r) => setTimeout(r, 250));
          ...
          if (fromCoords && window.L && mapRef.current) {
            ...
            fromMarker.bindPopup(`<strong>${pool.user_name}</strong><br/>${pool.from_location} → ${pool.to_location}<br/><span style="color:#888">${ratingText}</span>`);
        ...
        if (bounds.length > 0 && mapRef.current) mapRef.current.fitBounds(bounds, { padding: [40, 40], maxZoom: 11 });
  ...
  return <View style={{ flex: 1 }}>
    ...
    {status === "loading" && <View style={styles.overlay} pointerEvents="none"><ActivityIndicator color={colors.indigo} /><Text style={styles.overlayText}>Locating {resolvedCount}/{Math.min(pools.length, 15)} routes…</Text></View>}
