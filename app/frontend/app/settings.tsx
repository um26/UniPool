  ...
  return <SafeAreaView style={styles.safe} edges={["top", "bottom"]}>
    ...
    {loading && !prefs ? <View style={styles.center}><ActivityIndicator color={colors.indigo} /><Text style={styles.muted}>Loading settings…</Text></View> : <ScrollView contentContainerStyle={styles.content}>
      ...
      <Section title="Notifications" sub="Choose what deserves your attention. In-app updates still appear in the Notifications centre unless a category is disabled." styles={styles}>
        {prefs ? <>
          <SettingRow icon="notifications-outline" title="Browser push" sub={push.permission === "denied" ? "Blocked by browser settings" : push.subscribed ? "Push subscription is active" : "Off on this device"} value={!!prefs.push_enabled && !!push.subscribed} onValueChange={async (value: boolean) => { if (value && !push.subscribed) await push.subscribe(); if (!value && push.subscribed) await push.unsubscribe(); patchPrefs({ push_enabled: value }); }} disabled={push.busy || push.permission === "denied"} styles={styles} colors={colors} />
          <SettingRow icon="mail-outline" title="Email notifications" sub="Important ride and match emails" value={prefs.email_enabled} onValueChange={(value: boolean) => patchPrefs({ email_enabled: value })} styles={styles} colors={colors} />
