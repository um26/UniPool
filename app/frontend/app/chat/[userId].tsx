...
const QUICK_REPLIES = [
  "Hi! Are you still looking to share?",
...
export default function ChatThread() {
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try { setMsgs(await api.getThread(userId)); }
    ...
    <SafeAreaView style={styles.safe} edges={["top", "bottom"]}>
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ flex: 1 }}>
        <View style={styles.header}>
          <Pressable accessibilityLabel="Go back" testID="chat-back" onPress={() => router.back()} hitSlop={12}>
          ...
          <View style={{ alignItems: "center", flex: 1 }}>
            ...
            <View style={styles.statusRow}>
              {otherTyping ? (
                ...
                <><View style={styles.onlineDot} /><Text style={styles.statusText}>Online</Text></>
              ) : presence.last_seen ? (
                <Text style={styles.statusText}>Last seen {timeAgo(presence.last_seen)}</Text>
              ) : null}
            </View>
          </View>
          <Pressable accessibilityLabel="Report or block user" testID="chat-more" onPress={() => setShowReport(true)} hitSlop={12}>
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <View style={styles.empty}>
                ...
                <Text style={styles.emptyText}>Coordinate the pickup, timing and meeting point here.</Text>
              </View>
            }
                  ...
                  <View style={styles.messageWrap}>
                    ...
                    {(showRead || showSent) && (
                      <View style={styles.receiptRow}>
                        <Ionicons name={showRead ? "checkmark-done" : "checkmark"} size={14} color={showRead ? colors.indigo : colors.muted} />
...
const makeStyles = (colors: any) => StyleSheet.create({
  ...
  headerName: { fontSize: FONT.lg, fontWeight: "800", color: colors.onSurface, maxWidth: 240 },
  ...
  messageWrap: { maxWidth: "78%", minWidth: 52, flexShrink: 1 },
  bubble: { width: "100%", borderRadius: RADIUS.lg, paddingHorizontal: 14, paddingVertical: 10 },
  ...
  bubbleText: { fontSize: FONT.base, color: colors.onSurface, flexShrink: 1 },
  ...
  typingBubble: { width: 72, flexDirection: "row", gap: 4, alignItems: "center", paddingVertical: 14 },
  ...
  charHint: { textAlign: "right", color: colors.muted, fontSize: 10, paddingHorizontal: SPACING.lg, paddingVertical: 4, backgroundColor: colors.surface },
});
