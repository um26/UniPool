...
type Draft = { from?: string; to?: string; date?: string; time?: string; genderPref?: "any" | "same"; companions?: number; totalSeats?: number; luggage?: string; notes?: string; saved_at?: string };
...
export default function PostRequestScreen() {
  ...
  useEffect(() => {
    ...
    storage.secureGet(DRAFT_KEY, null).then((raw) => {
      ...
      if (draft && (draft.from || draft.to)) {
        ...
        setGenderPref(draft.genderPref || "any"); setCompanions(Number(draft.companions || 0)); setTotalSeats(Number(draft.totalSeats || 4)); setLuggage(draft.luggage || ""); setNotes(draft.notes || "");
  ...
  useEffect(() => {
    ...
    const handle = setTimeout(() => {
      if (!from.trim() && !to.trim() && !notes.trim() && !luggage.trim()) { storage.secureRemove(DRAFT_KEY); return; }
      const draft: Draft = { from, to, date, time, genderPref, companions, totalSeats, luggage, notes, saved_at: new Date().toISOString() };
      storage.secureSet(DRAFT_KEY, JSON.stringify(draft));
    }, 350);
  ...
  useEffect(() => { const handle = setTimeout(() => { if (from.trim().length >= 2) api.searchLocations(from.trim(), 5).then(setFromSuggestions).catch(() => setFromSuggestions([])); else setFromSuggestions([]); }, 180); return () => clearTimeout(handle); }, [from]);
  useEffect(() => { const handle = setTimeout(() => { if (to.trim().length >= 2) api.searchLocations(to.trim(), 5).then(setToSuggestions).catch(() => setToSuggestions([])); else setToSuggestions([]); }, 180); return () => clearTimeout(handle); }, [to]);
  ...
  const useRecent = (route: RecentRoute) => { setFrom(route.from_location); setTo(route.to_location); if (route.total_seats) setTotalSeats(route.total_seats); if (route.luggage) setLuggage(route.luggage); Haptics.selectionAsync(); };
  const discardDraft = () => { setFrom(params.from || ""); setTo(params.to || ""); setDate(defaultIst.date); setTime(defaultIst.time); setGenderPref("any"); setCompanions(0); setTotalSeats(4); setLuggage(""); setNotes(""); setDraftRestored(false); storage.secureRemove(DRAFT_KEY); };

  const rememberRecent = async (payload: any) => {
    const next: RecentRoute[] = [{ from_location: payload.from_location, to_location: payload.to_location, total_seats: payload.total_seats, luggage: payload.luggage, used_at: new Date().toISOString() }, ...recentRoutes.filter((r) => `${r.from_location}::${r.to_location}`.toLowerCase() !== `${payload.from_location}::${payload.to_location}`.toLowerCase())].slice(0, 5);
  ...
  const doSubmit = async (payload: any) => {
    setSubmitting(true);
    try {
      if (isEditing) await api.updatePool(params.edit as string, payload); else { await api.createPool(payload); await rememberRecent(payload); }
  ...
  return <SafeAreaView style={styles.safe} edges={["top", "bottom"]}><KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ flex: 1 }}>
    ...
    <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      {recentRoutes.length > 0 && <View style={styles.block}><Text style={styles.label}>Recent & repeat</Text><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 7 }}>{recentRoutes.map((route, index) => <Pressable key={`${route.from_location}-${route.to_location}`} onPress={() => useRecent(route)} style={styles.quickChip}><Ionicons name={index === 0 ? "repeat" : "time-outline"} size={13} color={colors.saffron} /><Text style={styles.quickText}>{route.from_location} → {route.to_location}</Text></Pressable>)}</ScrollView></View>}
      ...
      <View style={styles.routeCard}><View style={styles.routeField}><View style={[styles.dot, { backgroundColor: colors.saffron }]} /><View style={{ flex: 1 }}><Text style={styles.label}>From</Text><TextInput value={from} onChangeText={setFrom} placeholder="Campus, airport, station…" placeholderTextColor={colors.muted} style={styles.routeInput} /></View></View>{fromSuggestions.length > 0 && <SuggestionList items={fromSuggestions} onPick={pickFrom} styles={styles} colors={colors} />}<View style={styles.routeDivider} /><View style={styles.routeField}><View style={[styles.dot, { backgroundColor: colors.indigo }]} /><View style={{ flex: 1 }}><Text style={styles.label}>To</Text><TextInput value={to} onChangeText={setTo} placeholder="Where are you going?" placeholderTextColor={colors.muted} style={styles.routeInput} /></View><Pressable onPress={() => { const temp = from; setFrom(to); setTo(temp); }} style={styles.swap}><Ionicons name="swap-vertical" size={18} color={colors.indigo} /></Pressable></View>{toSuggestions.length > 0 && <SuggestionList items={toSuggestions} onPick={pickTo} styles={styles} colors={colors} />}</View>
      ...
      <View style={styles.row}><Counter label="Companions" value={companions} min={0} max={Math.max(0, totalSeats - 1)} onChange={setCompanions} styles={styles} colors={colors} /><Counter label="Total seats" value={totalSeats} min={Math.max(1, companions + 1)} max={8} onChange={setTotalSeats} styles={styles} colors={colors} /></View><View style={styles.seatHint}><Ionicons name="people" size={15} color={colors.success} /><Text style={styles.help}>{Math.max(0, totalSeats - companions - 1)} seat{Math.max(0, totalSeats - companions - 1) === 1 ? "" : "s"} available for UniPool travellers when you post.</Text></View>
...
const makeStyles = (colors: any) => StyleSheet.create({
  ...
  readiness: { backgroundColor: colors.surface2, borderRadius: RADIUS.lg, borderWidth: 1, borderColor: colors.border, padding: 14, marginBottom: 18 }, readinessTop: { flexDirection: "row", justifyContent: "space-between", alignItems: "center" }, eyebrow: { color: colors.muted, fontSize: 9, fontWeight: "900", letterSpacing: 1 }, readinessTitle: { color: colors.onSurface, fontSize: 16, fontWeight: "900", marginTop: 3 }, readinessScore: { color: colors.saffron, fontSize: 20, fontWeight: "900" }, track: { height: 6, borderRadius: 3, backgroundColor: colors.border, marginTop: 10, overflow: "hidden" }, fill: { height: "100%", backgroundColor: colors.saffron }, block: { marginBottom: 18 }, label: { color: colors.muted, fontSize: 10, fontWeight: "900", letterSpacing: .5, marginBottom: 6, textTransform: "uppercase" }, help: { color: colors.muted, fontSize: 10, lineHeight: 15, marginTop: 5 },
  quickChip: { flexDirection: "row", alignItems: "center", gap: 5, backgroundColor: colors.card, borderWidth: 1, borderColor: colors.border, borderRadius: 15, paddingHorizontal: 10, paddingVertical: 7, maxWidth: 260 }, placeChip: { flexDirection: "row", alignItems: "center", gap: 5, backgroundColor: colors.surface2, borderWidth: 1, borderColor: colors.border, borderRadius: 15, paddingHorizontal: 10, paddingVertical: 7 }, quickText: { color: colors.onSurface, fontSize: 10, fontWeight: "800" },
