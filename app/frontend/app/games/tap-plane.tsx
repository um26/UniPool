...
export default function TapPlane() {
  ...
  const spawn = () => {
    ...
    const maxY = AREA_HEIGHT - PLANE - 16;
    x.value = withTiming(Math.random() * maxX, { duration: 0 });
    y.value = withTiming(Math.random() * maxY, { duration: 0 });
  };
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <LinearGradient colors={["#0D47A1", COLORS.indigo]} style={styles.header}>
      ...
      <View style={styles.arena}>
        <LinearGradient colors={["#E3F2FD", "#BBDEFB", "#90CAF9"]} style={StyleSheet.absoluteFill} />
        {[...Array(6)].map((_, i) => (
          <View key={i} style={[styles.cloud, { top: (i * 90) % AREA_HEIGHT, left: (i * 60 + 20) % (width - 60) }]} />
        ))}
        {running && (
          <Animated.View style={[{ position: "absolute" }, style]}>
            <Pressable testID="tap-plane" onPress={tap} style={styles.plane}>
              <Ionicons name="airplane" size={44} color={COLORS.indigo} />
            </Pressable>
          </Animated.View>
        )}
        {!running && (
          <View style={styles.overlay}>
            <Ionicons name="airplane" size={64} color={COLORS.indigo} />
            <Text style={styles.overlayTitle}>{timeLeft === 30 ? "Tap-the-Plane" : `Time up! Score ${score}`}</Text>
            <Text style={styles.overlaySub}>Tap as many planes as you can in 30 seconds.</Text>
...
const styles = StyleSheet.create({
  ...
  stat: { flexDirection: "row", alignItems: "center", gap: 4, backgroundColor: "rgba(255,255,255,0.15)", paddingHorizontal: 10, paddingVertical: 4, borderRadius: RADIUS.pill },
  ...
  cloud: { position: "absolute", width: 60, height: 20, borderRadius: 20, backgroundColor: "rgba(255,255,255,0.7)" },
  ...
  overlay: { flex: 1, alignItems: "center", justifyContent: "center", padding: SPACING.xl, backgroundColor: "rgba(227,242,253,0.6)" },
  overlayTitle: { fontSize: FONT.xl, fontWeight: "800", color: COLORS.indigo, marginTop: SPACING.md },
