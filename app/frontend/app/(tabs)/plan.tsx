...
type Journey = {
  ...
  phase?: string; countdown_minutes?: number; is_owner?: boolean;
...
export default function ExploreScreen() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    const results = await Promise.allSettled([api.upcomingJourneys(), api.savedRoutes(), api.recurringRoutes(), api.routeInsights(), api.travelDigest(), api.diagnostics()]);
    const val = (i: number, fallback: any) => results[i].status === "fulfilled" ? (results[i] as PromiseFulfilledResult<any>).value : fallback;
    setJourneys(val(0, [])); setSaved(val(1, [])); setRecurring(val(2, [])); setInsights(val(3, [])); setDigest(val(4, null)); setDiagnostics(val(5, null));
  ...
  const saveRoute = async () => {
    ...
    setSavingRoute(true);
    try { await api.saveRoute({ from_location: saveFrom.trim(), to_location: saveTo.trim(), alerts_enabled: true, time_window_minutes: 180 }); setShowSave(false); Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success); await load(); }
    catch (e: any) { Alert.alert("Couldn't save route", e.message || "Try again"); } finally { setSavingRoute(false); }
  };
  const addRecurring = async () => {
    ...
    try { await api.createRecurringRoute({ from_location: repeatFrom.trim(), to_location: repeatTo.trim(), weekday, hour: Number(match[1]), minute: Number(match[2]), total_seats: 4, companions: 0, gender_preference: "any", active: true }); setShowRecurring(false); Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success); await load(); }
    catch (e: any) { Alert.alert("Couldn't create recurring ride", e.message || "Try again"); } finally { setSavingRepeat(false); }
  };
  const removeSaved = async (id: string) => { try { await api.deleteSavedRoute(id); setSaved((items) => items.filter((item) => item.saved_route_id !== id)); } catch (e: any) { Alert.alert("Couldn't remove route", e.message); } };
  const toggleRecurring = async (route: RecurringRoute) => { try { await api.updateRecurringRoute(route.template_id, { active: !route.active }); await load(); } catch (e: any) { Alert.alert("Couldn't update recurring ride", e.message); } };
  ...
  return <SafeAreaView style={styles.safe} edges={["top"]}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    ...
    <Section title="Next journey" sub={nextJourney ? "Trip Mode becomes useful when a ride is real." : "Your confirmed/upcoming rides will appear here."}>
      {nextJourney ? <Pressable onPress={() => go(`/pool/${nextJourney.pool_id}`)} style={styles.heroTrip}>
        ...
        <Text numberOfLines={1} style={styles.routeBig}>{nextJourney.from_location}</Text><View style={styles.routeArrow}><View style={styles.routeLine} /><Ionicons name="arrow-forward" size={16} color={colors.saffron} /></View><Text numberOfLines={1} style={styles.routeBig}>{nextJourney.to_location}</Text>
        <View style={styles.statRow}><MiniStat icon="people" text={nextJourney.seats ? `${nextJourney.seats.available} seats left` : "Seat details"} colors={colors} /><MiniStat icon="location" text={nextJourney.meeting_point?.label || "Set meeting point"} colors={colors} /><MiniStat icon="cash" text={nextJourney.fare?.per_person ? `₹${nextJourney.fare.per_person}/person` : "Add fare"} colors={colors} /></View>
    ...
    <Section title="Route intelligence" sub="Patterns from recent UniPool travel activity."><View style={styles.toolGrid}>{insights.slice(0, 4).map((item) => <View key={item.route_key} style={styles.insightCard}><Ionicons name="analytics" size={19} color={colors.saffron} /><Text numberOfLines={2} style={styles.cardTitle}>{item.from_location} → {item.to_location}</Text><Text style={styles.cardSub}>{item.trips_120d} trips · {item.peak_hour == null ? "building a pattern" : `peak around ${timeLabel(item.peak_hour)}`}</Text></View>)}{insights.length === 0 ? <Text style={styles.muted}>Route trends will appear as the network grows.</Text> : null}</View></Section>

    <Section title="Useful right now"><View style={styles.toolGrid}><Tool icon="search-outline" title="Find a ride" sub="Browse matching journeys" onPress={() => go("/(tabs)")} colors={colors} styles={styles} /><Tool icon="flame-outline" title="Route demand" sub="See busy corridors" onPress={() => go("/heatmap")} colors={colors} styles={styles} accent /><Tool icon="chatbubbles-outline" title="Trip chats" sub="Coordinate journeys" onPress={() => go("/(tabs)/messages")} colors={colors} styles={styles} /><Tool icon="shield-checkmark-outline" title="Student ID" sub="Identity and trust" onPress={() => go("/(tabs)/profile")} colors={colors} styles={styles} /></View></Section>

    <Section title="Time-pass" sub="Travel knowledge and word games — no random arcade clutter." action="See all" onAction={() => go("/(tabs)/games")}><View style={styles.gameRow}><GameShortcut icon="bulb" title="Travel Trivia" onPress={() => go("/games/trivia")} colors={colors} styles={styles} /><GameShortcut icon="text" title="Word Scramble" onPress={() => go("/games/word-scramble")} colors={colors} styles={styles} /><GameShortcut icon="airplane" title="Airport Codes" onPress={() => go("/games/airport-codes")} colors={colors} styles={styles} /><GameShortcut icon="search" title="Destination Detective" onPress={() => go("/games/destination-detective")} colors={colors} styles={styles} /><GameShortcut icon="ticket" title="Travel Reveal" onPress={() => go("/games/travel-reveal")} colors={colors} styles={styles} /></View></Section>
...
const makeStyles = (colors: any, isDark: boolean) => StyleSheet.create({
  safe: { flex: 1, backgroundColor: colors.surface }, content: { width: "100%", maxWidth: 980, alignSelf: "center", padding: SPACING.lg, paddingBottom: 130 },
  headingRow: { flexDirection: "row", alignItems: "flex-start", gap: 16, marginBottom: 28 }, eyebrow: { color: colors.saffron, fontWeight: "900", fontSize: 9, letterSpacing: 1.2 }, title: { color: colors.onSurface, fontSize: 30, fontWeight: "900", fontFamily: FONT_DISPLAY, marginTop: 4 }, subtitle: { color: colors.muted, fontSize: 12, lineHeight: 18, marginTop: 5, maxWidth: 620 },
  postButton: { minHeight: 42, borderRadius: 21, paddingHorizontal: 15, backgroundColor: colors.indigo, flexDirection: "row", alignItems: "center", gap: 6 }, postButtonText: { color: "#fff", fontWeight: "900", fontSize: 12 },
  loadingCard: { minHeight: 110, borderRadius: RADIUS.lg, backgroundColor: colors.card, borderWidth: 1, borderColor: colors.border, alignItems: "center", justifyContent: "center", gap: 8, marginBottom: 20 }, infoCard: { minHeight: 70, borderRadius: RADIUS.lg, backgroundColor: colors.card, borderWidth: 1, borderColor: colors.border, padding: 14, flexDirection: "row", alignItems: "center", gap: 10, marginBottom: 20 },
  heroTrip: { backgroundColor: colors.card, borderWidth: 1, borderColor: colors.border, borderRadius: 22, padding: 18 }, heroTop: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", gap: 8, marginBottom: 15 }, phasePill: { flexDirection: "row", alignItems: "center", gap: 6, backgroundColor: colors.surface2, borderRadius: 15, paddingHorizontal: 9, paddingVertical: 5 }, liveDot: { width: 6, height: 6, borderRadius: 3, backgroundColor: colors.success }, phaseText: { color: colors.success, fontSize: 9, fontWeight: "900", letterSpacing: .5 }, when: { color: colors.muted, fontSize: 10, fontWeight: "700" }, routeBig: { color: colors.onSurface, fontSize: 18, fontWeight: "900", fontFamily: FONT_DISPLAY }, routeArrow: { flexDirection: "row", alignItems: "center", gap: 8, height: 27 }, routeLine: { flex: 1, height: 1, backgroundColor: colors.border }, statRow: { flexDirection: "row", gap: 14, flexWrap: "wrap", marginTop: 16, borderTopWidth: 1, borderTopColor: colors.border, paddingTop: 13 }, openRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginTop: 14 }, openText: { color: colors.indigo, fontSize: 11, fontWeight: "900" },
  emptyCard: { flexDirection: "row", alignItems: "center", gap: 12, borderRadius: RADIUS.lg, backgroundColor: colors.card, borderWidth: 1, borderColor: colors.border, padding: 15 }, formCard: { backgroundColor: colors.surface2, borderRadius: RADIUS.lg, borderWidth: 1, borderColor: colors.border, padding: 12, gap: 9, marginBottom: 10 }, input: { backgroundColor: colors.card, borderWidth: 1, borderColor: colors.border, borderRadius: 12, paddingHorizontal: 12, paddingVertical: 11, color: colors.onSurface, fontSize: 12 }, primaryBtn: { minHeight: 42, borderRadius: 21, backgroundColor: colors.indigo, alignItems: "center", justifyContent: "center", flexDirection: "row", gap: 7, paddingHorizontal: 14 }, primaryBtnText: { color: "#fff", fontWeight: "900", fontSize: 11 }, stack: { gap: 8 }, routeCard: { flexDirection: "row", alignItems: "center", gap: 10, backgroundColor: colors.card, borderWidth: 1, borderColor: colors.border, borderRadius: RADIUS.lg, padding: 13 }, routeCardTop: { flexDirection: "row", alignItems: "center", gap: 7 }, alertPill: { flexDirection: "row", gap: 3, alignItems: "center", backgroundColor: colors.surface2, borderRadius: 10, paddingHorizontal: 6, paddingVertical: 3 }, alertText: { color: colors.success, fontWeight: "900", fontSize: 8 },
  dayChip: { borderRadius: 15, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.card, paddingHorizontal: 10, paddingVertical: 7 }, dayChipActive: { backgroundColor: colors.indigo, borderColor: colors.indigo }, dayText: { color: colors.onSurface, fontWeight: "800", fontSize: 10 },
  toolGrid: { flexDirection: "row", flexWrap: "wrap", gap: 10 }, toolCard: { flexGrow: 1, flexBasis: 190, minHeight: 120, backgroundColor: colors.card, borderRadius: RADIUS.lg, borderWidth: 1, borderColor: colors.border, padding: 14 }, toolIcon: { width: 39, height: 39, borderRadius: 13, backgroundColor: colors.surface2, alignItems: "center", justifyContent: "center", marginBottom: 10 }, insightCard: { flexGrow: 1, flexBasis: 205, minHeight: 105, backgroundColor: colors.card, borderWidth: 1, borderColor: colors.border, borderRadius: RADIUS.lg, padding: 13, gap: 7 },
  gameRow: { flexDirection: "row", flexWrap: "wrap", gap: 9 }, gameCard: { flexGrow: 1, flexBasis: 150, minHeight: 110, backgroundColor: isDark ? colors.surface2 : colors.cream, borderWidth: 1, borderColor: colors.border, borderRadius: RADIUS.lg, padding: 13, justifyContent: "space-between" }, gameIcon: { width: 36, height: 36, borderRadius: 12, backgroundColor: colors.card, alignItems: "center", justifyContent: "center" }, gameTitle: { color: colors.onSurface, fontWeight: "900", fontSize: 12, marginVertical: 8 }, bottomGrid: { flexDirection: "row", flexWrap: "wrap", gap: 10 }, digestCard: { flexGrow: 1, flexBasis: 260, minHeight: 105, backgroundColor: colors.card, borderRadius: RADIUS.lg, borderWidth: 1, borderColor: colors.border, padding: 14 }, cardTitle: { color: colors.onSurface, fontSize: 12, fontWeight: "900" }, cardSub: { color: colors.muted, fontSize: 10, lineHeight: 15, marginTop: 3 }, muted: { color: colors.muted, fontSize: 11 },
});
